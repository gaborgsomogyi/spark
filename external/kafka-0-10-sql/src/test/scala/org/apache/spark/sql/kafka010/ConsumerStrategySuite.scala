...
class ConsumerStrategySuite extends SparkFunSuite {
  ...
  private def consumer(strategy: ConsumerStrategy): Consumer[Array[Byte], Array[Byte]] = {
    strategy.createConsumer(consumerProps)
  }
  ...
  test("createConsumer must create consumer properly") {
    val strategy = AssignStrategy(Array.empty)
    assert(strategy.createConsumer(consumerProps) != null)
  }
  ...
  test("AssignStrategy.assignedTopicPartitions must give back all assigned") {
    ...
    assert(strategy.assignedTopicPartitions(consumer(strategy)) === partitions.toSet)
  ...
  test("AssignStrategy.assignedTopicPartitions must skip invalid partitions") {
    ...
    val strategy = AssignStrategy(partitions)
    assert(strategy.assignedTopicPartitions(consumer(strategy)) === Set.empty)
  ...
  test("SubscribeStrategy.assignedTopicPartitions must give back all assigned") {
    ...
    assert(strategy.assignedTopicPartitions(consumer(strategy)) === partitions)
